...
class Scheduler:
    ...
    def _prefill_waiting(self, sampling_params: SamplingParams):
        ...
        # Sync max_tokens from current sampling_params in case
        # add_request was called without sampling_params
    ...
    def _decode_running(self, sampling_params: SamplingParams):
        ...
        # One GPU call for all running sequences
        tokens = self.engine.decode_batch(self.running, sampling_params)

        still_running = []
        for seq, token in zip(self.running, tokens):
            seq.output_token_ids.append(token)
            if (token == self.tokenizer.eos_token_id
                    or seq.num_generated >= seq.max_tokens):
                seq.status = SequenceStatus.FINISHED
                self.finished.append(seq)
            else:
    ...
    def run_to_completion(self, sampling_params: SamplingParams = None) -> list[str]:
        ...
        return [
            self.tokenizer.decode(s.output_token_ids)
